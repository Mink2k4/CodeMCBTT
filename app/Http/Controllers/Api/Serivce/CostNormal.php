...
class CostNormal extends Controller
{
    ...
    {
        if($request->source == "Smm(Quantity-0%)"){
            $key = 'af09a033cb10e544c67fd95ef711c8f5';
            $action = 'services';
    ...
    public function CreateOrder($request, $link, $quantity, $service, $total_payment)
    {
        ...
        $server = ServerService::where('domain', getDomain())
                               ...
                               ->where(function($query) use ($request) {
                                   $query->where('id', $request->server_service)
                                         ->orWhere('actual_service', 'Smm(Quantity-0%)');
                               })
                               ->first();
        ...
        Log::info('Payload gửi:', $payload);
        \Log::info('Request đến API:', $request->all());
        // Gán các trường đặc biệt nếu có
        ...
        if (!empty($request->usernames)) {
            $payload['usernames'] = $request->usernames;
        }
        ...
        if (!empty($request->runs)) {
            $payload['runs'] = $request->runs;
        }
        if (!empty($request->interval)) {
            $payload['interval'] = $request->interval;
        }
        
        // Nếu không có bất kỳ trường đặc biệt nào ở trên thì truyền quantity
        if (
            empty($request->comments) &&
            empty($request->keywords) &&
            empty($request->usernames) &&
            empty($request->hashtag) &&
            empty($request->username) &&
            empty($request->media) &&
            empty($request->runs) &&
            empty($request->interval)
        ) {
            $payload['quantity'] = (int) $quantity;
        }
        // Gửi API
        $response = Http::post('https://smmcost.com/api/v2', $payload);
        ...
        if ($response->successful()) {
            ...
            return [
                'status' => false,
                'message' => "Lỗi từ API: " . $response->status() . " - " . $response->body()
            ];
        }
    }
}